...
void all_paths_N(int vertexList[], Edge edgeList[2*N_EDGES], int sourceNode, int destNode, int maxHops);
...
Path computeBackupPath(int vertexList[], Edge edgeList[2*N_EDGES], Path primary, Path pathToAvoid, int maxHops);
...
bool single_path_N_hops(int vertexList[], Edge edgeList[2*N_EDGES],int sourceNode, int destNode,int hops, int paths[NUM_CONNECTIONS][N_NODES], int connectionNum, Path structPaths[NUM_CONNECTIONS]) {
    ...
    //edgeListIndex[i] contains the index into edgeList[] (aka the compact adj list) for node i.
    int edgeListIndex[N_NODES];
    int elParent[N_NODES];
    int elIndex = 0;
    structPaths[connectionNum].index = 0;
    ...
    while(st.size() > 0) {
        ...
        for(; edgeListIndex[currentNode] < vertexList[currentNode+1]; ++edgeListIndex[currentNode]) {
            ...
            if(neighbor == destNode && currentHop == hops-1) {
                ...
                visited[neighbor] = 1;
                numPaths++;
                elParent[elIndex] = edgeListIndex[currentNode];

                structPaths[connectionNum].edges[structPaths[connectionNum].index] = &edgeList[edgeListIndex[currentNode]];
                //structPaths[connectionNum].index += 1;

                //Store the path, this is a hot mess
                //TODO:REVISE
                int reversePath[hops+1];
                int i = neighbor;
                int c = 0;
                reversePath[c] = i;
                ...
                c--;
                for(; c >= 0; --c) {
                    paths[connectionNum][hops-c] = reversePath[c];
                }

                for(int i = 0; i <= elIndex; ++i) {
                    cout << edgeList[elParent[i]].v1 << " -> " << edgeList[elParent[i]].v2 << "\n";

                    edgeList[elParent[i]].load++;
                }
                structPaths[connectionNum].sourceNode = sourceNode;
                structPaths[connectionNum].destNode = destNode;
                structPaths[connectionNum].hops = hops;
                return true;
            }
            ...
            if(!visited[neighbor]) {
                ...
                //TODO: EITHER OVERRIDE THE = ASIGNMENT IN THE STRUCT, OR DO EVERYTHING BY REFERENCE... REFERENCE IS PROBABLY BEST
                //cout << "STORING: " << edgeList[edgeListIndex[currentNode]].v1 << " -> " << edgeList[edgeListIndex[currentNode]].v2 << "\n";
                structPaths[connectionNum].edges[structPaths[connectionNum].index] = &edgeList[edgeListIndex[currentNode]];
                structPaths[connectionNum].index += 1;
                //cout << "STORED: " << (*structPaths[connectionNum].edges[structPaths[connectionNum].index-1]).v1 << " -> " << (*structPaths[connectionNum].edges[structPaths[connectionNum].index-1]).v2 << "\n";
...
Path computeBackupPath(int vertexList[], Edge edgeList[2*N_EDGES], Path primary, Path pathToAvoid, int maxHops) {
    ...
    st.push(primary.sourceNode);
    visited[primary.sourceNode] = 1;
    ...
    while(st.size() > 0) {
        ...
        for(; edgeListIndex[currentNode] < vertexList[currentNode+1]; ++edgeListIndex[currentNode]) {
            ...
            //Check to make sure we aren't avoiding this edge
            bool ruinedEdge = false;
            for(int k = 0; k < pathToAvoid.index; ++k) {
                if((*pathToAvoid.edges[k]).v1 == currentNode && (*pathToAvoid.edges[k]).v2 == neighbor) {
                    ruinedEdge = true;
                }
            }

            //If we are avoiding that edge, skip it!
            if(ruinedEdge == true) {
                continue;
            }
            /* add this to restrict the path length
            if(neighbor == primary.destNode && currentHop != maxHops-1) {
                continue;
            }*/

            //If our neighbor is the desired node, print out the current path.
            if(neighbor == primary.destNode) {
                visited[neighbor] = 1;

                hopefulPath.edges[hopefulPath.index] = &edgeList[edgeListIndex[currentNode]];
                hopefulPath.sourceNode = primary.sourceNode;
                hopefulPath.destNode = primary.destNode;
                hopefulPath.hops = currentHop;
...
void all_paths_N(int vertexList[], Edge edgeList[2*N_EDGES], int sourceNode, int destNode, int maxHops) {
    ...
    while(st.size() > 0) {
        ...
        for(; edgeListIndex[currentNode] < vertexList[currentNode+1]; ++edgeListIndex[currentNode]) {
            ...
            //If our neighbor is the desired node, print out the current path.
            if(neighbor == destNode && currentHop == maxHops-1) {
                parent[neighbor] = currentNode;
                visited[neighbor] = 1;
                numPaths++;
                //printf("#HOPS: %d\n",currentHop + 1);
                //printPath(sourceNode,destNode,parent);
                //We continue because there may be a roundabout path from the currentNode to the destination Node.
                continue;
            }
    ...
    cout << "HOPS: " << maxHops << ", NUM PATHS: " << numPaths << "\n";
    ofstream outputFile;
    outputFile.open("output.csv",ios_base::app);
    outputFile << sourceNode << "," << destNode << "," << maxHops << "," << numPaths << "\n";
    outputFile.close();
}
